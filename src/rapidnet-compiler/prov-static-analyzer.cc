...
NS_LOG_COMPONENT_DEFINE ("ProvenanceStaticAnalyzer");
  ...
  bool provenanceEnabled)
{
  ...
  // Static Analyze
  cout << "Static Analyzing..." << endl;
  Ptr<AnalyzerContext> actxt (new AnalyzerContext ());
  if (actxt->Analyze (ctxt, tableStore))
    {
      printToFile (overlogFile + ".sta", actxt->ToString ());
    }
...
int main (int argc, char** argv)
{
  ...
  LogComponentEnable ("AnalyzerContext", LOG_LEVEL_INFO);
  LogComponentEnable ("ProvenanceStaticAnalyzer", LOG_LEVEL_INFO);
  LogComponentEnable ("VariableGraph", LOG_LEVEL_INFO);
