...
* 
* This version uses the exact matching scheme. This means the 
* equivalence keys are all the attributes in the event tuple.
*
*****************/
...
prov_ri1 initPacketHash(@Node, SrcAdd, DstAdd, Data, PIDequi, ProgID) :-
    ...
    PIDequi := f_sha1("" + Node + SrcAdd + DstAdd + Data).
...
ro2 provRef(@Node, PID, EquiHash, PIDev) :-
    recvPacketProv(@Node, SrcAdd, DstAdd, Data, Tag),
...
rho3 provRef(@Node, PID, EquiHash, PIDev) :-
...
rqini rQuery(@Loc,NQID,RID,Count,PIDev,Ret) :-
 provQuery(@X,QID,PID,Ret),
 provRef(@X,PID,EquiHash,PIDev),
 provHashTable(@X,EquiHash,ProgID,Loc,RID,Count),
 NQID := f_sha1(""+QID+RID).

/* Query the event tuple*/
rqev tQuery(@X,NNQID,PIDev,TupleRet) :-
 rQuery(@X,NQID,RID,Count,PIDev,Ret),
...
rqrec rQuery(@Preloc,NNQID,PreRID,PreCount,PIDev,X) :-
 rQuery(@X,NQID,RID,Count,PIDev,Ret),
 provLink(@X,RID,CurCount,Preloc,PreRID,PreCount),
 PreRID != f_sha1("NULL"),
