...
prov_rs1_1 epacketTemp(@RLOC, Next, SrcAdd, DstAdd, Data, RID, R, List, Tag) :-
    ...
    pktProv(@Switch, SrcAdd, DstAdd, Data, Tag),
...
prov_rs1_4 pktProv(@Next, SrcAdd, DstAdd, Data, NewTag) :-
    epacketTemp(@RLOC, Next, SrcAdd, DstAdd, Data, RID, R, List, Tag),
    Hash := f_append(RID),
    NewTag := f_concat(Tag, Hash).

/* Rules for normal execution without provenance */
/* "P" in packetP is for padding for fair bandwidth*/
rs1 packetP(@Next, SrcAdd, DstAdd, Data, PIDHash) :-
        device(@Switch, Dvtype),
        packetP(@Switch, SrcAdd, DstAdd, Data, PIDHash),
 flowEntry(@Switch, DstAdd, Next),
 link(@Switch, Next),
        Dvtype == 0 .
...
rh103 packetP(@Switch, SrcAdd, DstAdd, Data, PIDHash) :-
    device(@Host, Dvtype),
    initPacketCount(@Host, SrcAdd, DstAdd, Data, PIDequi, PIDcount),
    linkhr(@Host, Switch),
    Dvtype == 1,
...
prov_rh2_1 erecvPacketTemp(@RLOC, Host, SrcAdd, DstAdd, Data, RID, R, List, Tag) :-
    device(@Host, Dvtype),
    pktProv(@Host, SrcAdd, DstAdd, Data, Tag),
    DstAdd == Host,
    Dvtype == 1,
    PID1 := f_sha1(("device"+ Host)+ Dvtype),
    List := f_append(PID1),
...
rh2 recvPacket(@Host, SrcAdd, DstAdd, Data, PIDequi, PIDev) :-
        device(@Host, Dvtype),
 packetP(@Host, SrcAdd, DstAdd, Data, PIDHash),
