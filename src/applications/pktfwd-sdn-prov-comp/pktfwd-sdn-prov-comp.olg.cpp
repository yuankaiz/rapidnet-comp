...
rs10 eMatchingPacketTemp(@RLoc, Switch, SrcAdd, DstAdd, Data, TopPriority, RID, RTempList, HashList) :-
    ...
    RID := f_sha1(R + List),
    Rlist := f_append(R),
    RTempList := f_concat(Rlist, List).

rs11 eMatchingPacket(@Switch, SrcAdd, DstAdd, Data, TopPriority, RID, RLoc, HashList) :-
    eMatchingPacketTemp(@RLoc, Switch, SrcAdd, DstAdd, Data, TopPriority, RID, RTempList, HashList).
...
rs20 eMatchingPacketTemp(@RLoc, Switch, SrcAdd, DstAdd, Data, NextPriority, RID, RTempList, HashList) :-
    device(@Switch, Dvtype),
