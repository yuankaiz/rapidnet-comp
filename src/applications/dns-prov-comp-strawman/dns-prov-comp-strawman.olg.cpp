...
prov_r1_1 eRequestTemp(@RLOC,X,URL,HOST,RID,R,List,HashList) :-
     url(@X,URL,HOST),
...
prov_r1_2 eRequestCount(@RLOC, RID, R, List, a_COUNT<*>):-
  eRequestTemp(@RLOC,X,URL,HOST,RID,R,List,HashList),
...
prov_r1_4 request(@X,URL,HOST,NewHashList) :-
    eRequestTemp(@RLOC,X,URL,HOST,RID,R,List,HashList),
     Hash := f_append(RID),
     NewHashList := f_concat(HashList, Hash).

/*Rule r2, traverse the DNS tree*/
prov_r2_1 eRequestTemp(@RLOC,ADDR,URL,HOST,RID,R,List,HashList) :-
  request(@X,URL,HOST,HashList),
...
prov_r3_1 eResultTemp(@RLOC,ADDR,URL,HOST,RID,R,List,HashList) :-
   request(@X,URL,HOST,HashList),
   name_server(@X,DOMAIN,SERVER),
   address_record(@X,SERVER,ADDR),
   f_indexof(URL,DOMAIN)!=-1,
...
prov_r3_2 eResultCount(@RLOC, RID, R, List, a_COUNT<*>) :-
    eResultTemp(@RLOC, ADDR, URL, HOST, RID, R, List, HashList),
    ruleExec(@RLOC, RID, R, List).
...
prov_r3_4 eResult(@ADDR,URL,HOST,RID,RLOC,HashList) :-
    eResultTemp(@RLOC,ADDR,URL,HOST,RID,R,List,HashList),
    Hash := f_append(RID),
    NewHashList := f_concat(HashList, Hash).

prov_r3_5 result(@ADDR,URL,HOST) :-
   eResult(@ADDR,URL,HOST,RID,RLOC,HashList).

prov_r3_6 resultAux(@ADDR,PID,HashList) :-
   eResult(@ADDR,URL,HOST,RID,RLOC,HashList),
   PID := f_sha1("result" + ADDR + URL + HOST).
