...
rs10 eMatchingPacketTemp(@RLoc, Switch, SrcAdd, DstAdd, TopPriority, RID, R, List, Tag) :-
    device(@Switch, Dvtype),
    packetProv(@Switch, SrcAdd, DstAdd, Tag),
...
rs11 eMatchingPacket(@Switch, SrcAdd, DstAdd, TopPriority, RID, RLoc, Tag) :-
    eMatchingPacketTemp(@RLoc, Switch, SrcAdd, DstAdd, TopPriority, RID, R, List, Tag).
...
rs14 matchingPacket(@Switch, SrcAdd, DstAdd, TopPriority, NewTag) :-
    eMatchingPacket(@Switch, SrcAdd, DstAdd, TopPriority, RID, RLoc, Tag),
    Hash := f_append(RID),
    NewTag := f_concat(Tag, Hash).

/* Rules for normal execution without provenance */
rs1 matchingPacketNonProv(@Switch, SrcAdd, DstAdd, TopPriority, PIDequi, PIDev) :-
        device(@Switch, Dvtype),
 packetNonProv(@Switch, SrcAdd, DstAdd, PIDequi, PIDev),
 maxPriority(@Switch, TopPriority),
        Dvtype == 0 .

/*Recursively matching entries in a routing table*/
rs20 eMatchingPacketTemp(@RLoc, Switch, SrcAdd, DstAdd, NextPriority, RID, R, List, Tag) :-
    device(@Switch, Dvtype),
    matchingPacket(@Switch, SrcAdd, DstAdd, Priority, Tag),
...
/*Recursively matching entries in a routing table*/
rs2 matchingPacketNonProv(@Switch, SrcAdd, DstAdd, NextPriority, PIDequi, PIDev) :-
        device(@Switch, Dvtype),
 matchingPacketNonProv(@Switch, SrcAdd, DstAdd, Priority, PIDequi, PIDev),
 flowEntry(@Switch, DstEntry, Next, Priority),
 Priority > 0,
 DstAdd != DstEntry,
 NextPriority := Priority - 1,
...
rs30 ePacketTemp(@RLoc, Next, SrcAdd, DstAdd, RID, R, List, Tag) :-
        device(@Switch, Dvtype),
        matchingPacket(@Switch, SrcAdd, DstAdd, Priority, Tag),
 flowEntry(@Switch, DstEntry, Next, Priority),
 link(@Switch, Next),
...
rs31 ePacket(@Next, SrcAdd, DstAdd, RID, RLoc, Tag) :-
 ePacketTemp(@RLoc, Next, SrcAdd, DstAdd, RID, R, List, Tag).
...
rs34 packetProv(@Next, SrcAdd, DstAdd, NewTag) :-
 ePacket(@Next, SrcAdd, DstAdd, RID, RLoc, Tag),
 Hash := f_append(RID),
 NewTag := f_concat(Tag, Hash).

/*A hit in the routing table, forward the packet accordingly*/
rs3 packetNonProv(@Next, SrcAdd, DstAdd, PIDequi, PIDev) :-
        device(@Switch, Dvtype),
        matchingPacketNonProv(@Switch, SrcAdd, DstAdd, Priority, PIDequi, PIDev),
 flowEntry(@Switch, DstEntry, Next, Priority),
 link(@Switch, Next),
...
rs40 ePacketTemp(@RLoc, Next, SrcAdd, DstAdd, RID, R, List, Tag) :-
        device(@Switch, Dvtype),
        matchingPacket(@Switch, SrcAdd, DstAdd, Priority, Tag),
...
/*No hit in the routing table, follow the default routing*/
rs4 packetNonProv(@Next, SrcAdd, DstAdd, PIDequi, PIDev) :-
        device(@Switch, Dvtype),
        matchingPacketNonProv(@Switch, SrcAdd, DstAdd, Priority, PIDequi, PIDev),
 flowEntry(@Switch, DstAdd, Next, Priority),
 link(@Switch, Next),
...
rh100 initPacketHash(@Host, SrcAdd, DstAdd, PIDequi) :-
    initPacket(@Host, SrcAdd, DstAdd),
    PIDequi := f_sha1("" + Host + DstAdd).

rh101 initPacketCount(@Host, SrcAdd, DstAdd, PIDequi, a_COUNT<*>) :-
    initPacketHash(@Host, SrcAdd, DstAdd, PIDequi),
    equiHashTable(@Host, DstAdd, PIDequi).

rh102 equiHashTable(@Host, DstAdd, PIDequi) :-
    initPacketCount(@Host, SrcAdd, DstAdd, PIDequi, PIDcount),
    PIDcount == 0.

rh103 packetNonProv(@Switch, SrcAdd, DstAdd, PIDequi, PIDev) :-
    device(@Host, Dvtype),
    initPacketCount(@Host, SrcAdd, DstAdd, PIDequi, PIDcount),
    linkhr(@Host, Switch),
    Dvtype == 1,
    PIDev := f_sha1("initPacket" + Host + SrcAdd + DstAdd), /*Generate event hash*/
    PIDcount != 0.

rh104 ePacketTemp(@RLoc, Switch, SrcAdd, DstAdd, RID, R, List, Tag) :-
        device(@Host, Dvtype),
        initPacketCount(@Host, SrcAdd, DstAdd, PIDequi, PIDcount),
 linkhr(@Host, Switch),
        Dvtype == 1,
...
rh20 eRecvPacketTemp(@RLoc, Host, SrcAdd, DstAdd, RID, R, List, Tag) :-
 device(@Host, Dvtype),
 packetProv(@Host, SrcAdd, DstAdd, Tag),
        Dvtype == 1,
 RLoc := Host,
 R := "rh2",
...
rh21 eRecvPacket(@Host, SrcAdd, DstAdd, RID, RLoc, Tag) :-
    eRecvPacketTemp(@RLoc, Host, SrcAdd, DstAdd, RID, R, List, Tag).
...
rh24 recvPacketTemp(@Host, SrcAdd, DstAdd, Tag) :-
 eRecvPacket(@Host, SrcAdd, DstAdd, RID, RLoc, Tag),
 Hash := f_append(RID),
 NewTag := f_concat(Tag, Hash).

rh25 recvPacketDecomp(@Host, SrcAdd, DstAdd, PIDequi, PIDev, Hashlist) :-
    recvPacketTemp(@Host, SrcAdd, DstAdd, Tag),
    PIDequi := f_first(Tag),
...
rh26 provHashTable(@Host, PIDequi, Hashlist) :-
    recvPacketDecomp(@Host, SrcAdd, DstAdd, PIDequi, PIDev, Hashlist).

rh27 recvPacket(@Host, SrcAdd, DstAdd, PIDequi, PIDev) :-
    recvPacketDecomp(@Host, SrcAdd, DstAdd, PIDequi, PIDev, Hashlist).

/* Process a packet that is NOT the first of the equivalence class*/
rh2 recvPacket(@Host, SrcAdd, DstAdd, PIDequi, PIDev) :-
        device(@Host, Dvtype),
 packetNonProv(@Host, SrcAdd, DstAdd, PIDequi, PIDev),
        Dvtype == 1 .
