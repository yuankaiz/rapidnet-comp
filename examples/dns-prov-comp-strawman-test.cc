...
#include "ns3/dns-prov-comp-strawman-module.h"
...
#define name_server_tuple(src,domain,server)\
  tuple(DnsProvCompStrawman::NAME_SERVER,\
...
#define address_record_tuple(src,server,address)\
  tuple(DnsProvCompStrawman::ADDRESS_RECORD,\
...
#define result_tuple(host,url,address)		\
  tuple(DnsProvCompStrawman::RESULT,\
...
void Print()
{
  ...
  //	cout<<endl;
  PrintRelation(apps,DnsProvCompStrawman::RESULT);
}
...
{
  LogComponentEnable("DnsProvCompStrawman", LOG_LEVEL_INFO);
  LogComponentEnable("RapidNetApplicationBase", LOG_LEVEL_INFO);

  apps = InitRapidNetApps (20, Create<DnsProvCompStrawmanHelper> ());
...
int main(int argc,char *argv[])
{
  ...
  string storePath = "/localdrive1/harshal/prov_test_strawman/";
   CommandLine cmd;
  ...
  Ptr<RapidNetApplicationHelper> appHelper = Create<DnsProvCompStrawmanHelper> ();
