...
#include "ns3/pktfwd-sdn-prov-module.h"
...
#define initpacket(host, srcadd, dstadd, data)               \
  tuple(PktfwdSdnProv::INITPACKET,\
...
#define maxPriority(sw, priority)\
  tuple(PktfwdSdnProv::MAXPRIORITY,\
...
void Print ()
{
  PrintRelation (apps, PktfwdSdnProv::RECVPACKET);
  PrintRelation (apps, PktfwdSdnProv::RULEEXEC);
  PrintRelation (apps, PktfwdSdnProv::PROV);
}
...
{
  LogComponentEnable("PktfwdSdnProv", LOG_LEVEL_INFO);
  ...
  //apps = InitRapidNetApps (totalNum, Create<PktfwdSdnProvHelper> ());
  /* CSMA device model*/
  ...
  Ptr<RapidNetApplicationHelper> appHelper = Create<PktfwdSdnProvHelper> ();
