...
#define associationrule(node, item, rule, probability)\
  ns3::rapidnet::tuple (Ml::ASSOCIATIONRULE, \
...
void Print ()
{
  PrintRelation (apps, Ml::IMAGE);
  PrintRelation (apps, Ml::PREDICTION);
  PrintRelation (apps, Ml::IDENTIFIEDPATTERN);
  PrintRelation (apps, Ml::RESULT);
}
...
main (int argc, char *argv[])
{
  ...
  string storepath = "output/ml/";
  std::ofstream ofs;
  ofs.open("output/ml/timing", std::ofstream::out | std::ofstream::trunc);
  ofs.close();

  apps = InitRapidNetApps (1, Create<MlHelper> ());
