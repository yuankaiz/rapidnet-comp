...
#include "ns3/pktfwd-norm-prov-oct0116-module.h"
...
#define link(sw, nei)\
  tuple(PktfwdNormProvOct0116::LINK,\
...
#define initpacket(host, srcadd, dstadd, data)         \
  tuple(PktfwdNormProvOct0116::INITPACKET,\
...
#define flowentry(sw, dst, next)		\
  tuple(PktfwdNormProvOct0116::FLOWENTRY,\
...
void Print ()
{
  PrintRelation (apps, PktfwdNormProvOct0116::RECVPACKET);
  PrintRelation (apps, PktfwdNormProvOct0116::PROV);
  PrintRelation (apps, PktfwdNormProvOct0116::RULEEXEC);
...
void InitApps(int nodeNum)
{
  ...
  apps.Add(Create<PktfwdNormProvOct0116Helper>()->Install(mainNodes));
...
{
  LogComponentEnable("PktfwdNormProvOct0116", LOG_LEVEL_INFO);
